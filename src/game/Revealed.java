...
public class Revealed extends Tile {
	 ...
	 * 
	 * @param board
	 *            a Board object that contains all the tiles
	 * @param i
	 *            the row to get around
	 * @param j
	 ...
	 * 
	 * @param isMine
	 *            a boolean is mine or not
	 * @param isHidden
	 *            a boolean is hidden or not
	 * @param isDefused
	...
	public void render(Graphics g, int x, int y) {
		g.drawImage(m_revealedImage.getImage(),
				x * Tile.WIDTH, y * Tile.HEIGHT, null);
		...
		if (m_nearbyMines > 0) {
			...
			g.drawString(Integer.toString(m_nearbyMines), x * Tile.WIDTH 
					+ spacingX, y * Tile.HEIGHT + spacingY);
		}
	 ...
	 * 
	 * @param tile
	 *            a Tile thats being checked
	 * @param tileArround
	 *            an ArrayList of tiles that are around the tile
	 */
